...
class GFPGANMultipleModel(BaseModel):
    ...
    def feed_data(self, data):
        self.lq = data[0]['lq'].to(self.device)
        self.lq_sub = torch.cat([data[i]['lq'] for i in range(1, self.opt['datasets']["train"]["max_img"])], dim=1).to(self.device)

        if 'gt' in data[0]:
            self.gt = data[0]['gt'].to(self.device)

        if 'loc_left_eye' in data[0]:
            # get facial component locations, shape (batch, 4)
            self.loc_left_eyes = data[0]['loc_left_eye']
            self.loc_right_eyes = data[0]['loc_right_eye']
            self.loc_mouths = data[0]['loc_mouth']
    ...
    def optimize_parameters(self, current_iter):
        ...
        if pyramid_loss_weight > 0:
            self.output, out_rgbs = self.net_g(self.lq, self.lq_sub, return_rgb=True)
            pyramid_gt = self.construct_img_pyramid()
        else:
            self.output, out_rgbs = self.net_g(self.lq, self.lq_sub, return_rgb=False)
    ...
    def test(self):
        with torch.no_grad():
            if hasattr(self, 'net_g_ema'):
                self.net_g_ema.eval()
                self.output, _ = self.net_g_ema(self.lq, self.lq_sub)
            else:
                logger = get_root_logger()
                logger.warning('Do not have self.net_g_ema, use self.net_g.')
                self.net_g.eval()
                self.output, _ = self.net_g(self.lq, self.lq_sub)
                self.net_g.train()
