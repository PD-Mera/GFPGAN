...
class SelfAttention2D(nn.Module):
    ...
    def forward(self,x):
        ...
        attention = self.softmax(scores) # BX (N) X (N) 
...
class GFPGANSelfAttentionv1(nn.Module):
    ...
    def __init__(
        ...
        super(GFPGANSelfAttentionv1, self).__init__()
        ...
        self.attn_latent = SelfAttention1D(channels['4'])

        # upsample
        in_channels = channels['4']
        self.conv_body_up = nn.ModuleList()
        self.sttn_up = nn.ModuleList()
        for i in range(3, self.log_size + 1):
            out_channels = channels[f'{2**i}']
            self.conv_body_up.append(ResUpBlock(in_channels, out_channels))
            self.sttn_up.append(SelfAttention2D(out_channels))
            in_channels = out_channels
    ...
    def forward(self, x, return_latents=False, return_rgb=True, randomize_noise=True, **kwargs):
        ...
        style_code = self.attn_latent(style_code)
